...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { aX: 'hidden', aK: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { aX: 'mozHidden', aK: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { aX: 'msHidden', aK: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { aX: 'webkitHidden', aK: 'webkitvisibilitychange' }
		: { aX: 'hidden', aK: 'visibilitychange' };
}
...
var author$project$Main$generateFoodPos = function (_n0) {
	var grid = _n0.h;
	var playerPos = _n0.j;
	var gridPos = A2(
		...
		function (x) {
			return A2(
				...
				A2(elm$core$List$range, 0, grid.C - 1));
		},
		A2(elm$core$List$range, 0, grid.H - 1));
	var possiblePos = elm$core$Set$toList(
		A2(
			elm$core$Set$diff,
			elm$core$Set$fromList(gridPos),
			elm$core$Set$fromList(playerPos)));
	if (!possiblePos.b) {
		return elm$core$Platform$Cmd$none;
	} else {
		var head = possiblePos.a;
		var tail = possiblePos.b;
		return A2(
...
var author$project$Main$init = function (_n0) {
	...
	return _Utils_Tuple2(
		model,
		author$project$Main$generateFoodPos(model));
};
var author$project$Main$Tick = {$: 1};
...
var author$project$Main$gameStateKeyDecoder = function (_n0) {
	var gameState = _n0.s;
	if (!gameState) {
		return A2(
				...
				switch (s) {
					case 'ArrowUp':
						return elm$json$Json$Decode$succeed(
							author$project$Main$Turn(0));
					case 'ArrowDown':
						return elm$json$Json$Decode$succeed(
							author$project$Main$Turn(1));
					case 'ArrowLeft':
						return elm$json$Json$Decode$succeed(
							author$project$Main$Turn(2));
					case 'ArrowRight':
						return elm$json$Json$Decode$succeed(
							author$project$Main$Turn(3));
					default:
		...
		return A2(
			...
			function (s) {
				if (s === ' ') {
					return elm$json$Json$Decode$succeed(author$project$Main$Restart);
				} else {
					return elm$json$Json$Decode$fail(':(');
				}
			},
...
var elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aj: fragment, al: host, ar: path, au: port_, ax: protocol, ay: query};
	});
...
var author$project$Main$subscriptions = function (model) {
	var gameState = model.s;
	if (!gameState) {
		return elm$core$Platform$Sub$batch(
			_List_fromArray(
				[
					elm$browser$Browser$Events$onKeyDown(
					author$project$Main$gameStateKeyDecoder(model)),
					A2(
					elm$time$Time$every,
					100,
					function (_n1) {
	...
	function (msg, model) {
		...
		switch (msg.$) {
			case 0:
				...
				return _Utils_eq(
					d,
					antonio(model.Q)) ? _Utils_Tuple2(model, elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{K: d}),
					elm$core$Platform$Cmd$none);
			case 1:
				var nextPlayerPos = function () {
					var _n5 = model.K;
					switch (_n5) {
						case 0:
							return _Utils_Tuple2(
								playerX,
								A2(elm$core$Basics$modBy, model.h.C, playerY - 1));
						case 1:
							return _Utils_Tuple2(
								playerX,
								A2(elm$core$Basics$modBy, model.h.C, playerY + 1));
						case 2:
							return _Utils_Tuple2(
								A2(elm$core$Basics$modBy, model.h.H, playerX - 1),
								playerY);
						default:
							return _Utils_Tuple2(
								A2(elm$core$Basics$modBy, model.h.H, playerX + 1),
				...
				var nextGameState = A2(elm$core$List$member, nextPlayerPos, model.j) ? 1 : model.s;
				var lunchTime = function () {
					var _n4 = model.n;
					if (!_n4.$) {
						var foodPos = _n4.a;
						return _Utils_eq(nextPlayerPos, foodPos);
					} else {
						return false;
					}
				}();
				var nextEatenFoods = function () {
					var _n3 = _Utils_Tuple2(model.n, lunchTime);
					if ((!_n3.a.$) && _n3.b) {
						var foodPos = _n3.a.a;
						return A2(elm$core$List$cons, foodPos, model.A);
					} else {
						return model.A;
					}
				}();
				var nextFoodPos = lunchTime ? elm$core$Maybe$Nothing : model.n;
				var tailDropSize = lunchTime ? 0 : 1;
				var cmd = lunchTime ? author$project$Main$generateFoodPos(model) : elm$core$Platform$Cmd$none;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							j: A2(
								...
								A2(
									elm$core$List$take,
									elm$core$List$length(model.j) - tailDropSize,
									model.j))
						}),
					cmd);
			case 2:
...
var author$project$Main$colorPalette = {
	ac: A3(avh4$elm_color$Color$rgb255, 40, 42, 54),
	aM: A3(avh4$elm_color$Color$rgb255, 98, 114, 164),
	aN: A3(avh4$elm_color$Color$rgb255, 139, 233, 253),
	aV: A3(avh4$elm_color$Color$rgb255, 248, 248, 242),
	ak: A3(avh4$elm_color$Color$rgb255, 80, 250, 123),
	a7: A3(avh4$elm_color$Color$rgb255, 255, 184, 108),
	X: A3(avh4$elm_color$Color$rgb255, 255, 121, 198),
	a8: A3(avh4$elm_color$Color$rgb255, 189, 147, 249),
	a9: A3(avh4$elm_color$Color$rgb255, 255, 85, 85),
	bb: A3(avh4$elm_color$Color$rgb255, 68, 71, 90),
	bk: A3(avh4$elm_color$Color$rgb255, 241, 250, 140)
};
				...
				switch (setting.$) {
					case 0:
						...
						return _Utils_update(
							r,
							{
								m: A2(elm$core$List$cons, cmd, r.m)
							});
					case 1:
						var cmds = setting.a;
						return _Utils_update(
							r,
							{
								m: A3(elm$core$List$foldl, elm$core$List$cons, r.m, cmds)
							});
					case 3:
						var f = setting.a;
						return _Utils_update(
...
var author$project$Main$viewTiles = F3(
	function (sideSize, tilePositions, color) {
		var floatSideSize = sideSize;
		var tileRect = function (_n0) {
			var x = _n0.a;
			var y = _n0.b;
			return A3(
				joakin$elm_canvas$Canvas$rect,
				_Utils_Tuple2(x * sideSize, y * sideSize),
				floatSideSize,
				floatSideSize);
		};
		var tileRects = A2(elm$core$List$map, tileRect, tilePositions);
		return A2(
...
var author$project$Main$gameStateView = function (_n0) {
	var gameState = _n0.s;
	var grid = _n0.h;
	var playerPos = _n0.j;
	var foodPos = _n0.n;
	var eatenFoods = _n0.A;
	if (!gameState) {
		return _List_fromArray(
			[
				A3(author$project$Main$viewTiles, grid.v, playerPos, author$project$Main$colorPalette.ak),
				A3(
				author$project$Main$viewTiles,
				grid.v,
				A2(
					elm$core$Maybe$withDefault,
					_List_Nil,
					A2(elm$core$Maybe$map, elm$core$List$singleton, foodPos)),
				author$project$Main$colorPalette.X)
			]);
	} else {
		return _List_fromArray(
			[
				A3(author$project$Main$viewTiles, grid.v, eatenFoods, author$project$Main$colorPalette.X)
			]);
	}
};
...
var joakin$elm_canvas$Canvas$renderTextFill = F5(
	function (txt, x, y, color, cmds) {
		return A2(
			elm$core$List$cons,
			A4(joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$fillText, txt.aa, x, y, txt.U),
			A2(
				elm$core$List$cons,
...
var joakin$elm_canvas$Canvas$renderTextStroke = F5(
	function (txt, x, y, color, cmds) {
		return A2(
			elm$core$List$cons,
			A4(joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$strokeText, txt.aa, x, y, txt.U),
			A2(
				elm$core$List$cons,
	...
	function (x, y, t, cmds) {
		return A2(
			...
			function () {
				if (!t.$) {
					var image = t.a;
					return A9(joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, 0, 0, image.H, image.C, x, y, image.H, image.C, image.a_);
				} else {
					var sprite = t.a;
					var image = t.b;
					return A9(joakin$elm_canvas$Canvas$Internal$CustomElementJsonApi$drawImage, sprite.R, sprite.S, sprite.H, sprite.C, x, y, sprite.H, sprite.C, image.a_);
				}
			}(),
			cmds);
	});
		...
		return A3(
			...
			F2(
				function (width, height) {
					return {C: height, a_: target, H: width};
				}),
			A2(
	...
	function (options, attrs, entities) {
		return A3(
			...
			A2(
				...
				A2(
					elm$core$List$cons,
					elm$html$Html$Attributes$height(options.C),
					A2(
						elm$core$List$cons,
						elm$html$Html$Attributes$width(options.H),
						attrs))),
			A2(
				elm$core$List$cons,
				_Utils_Tuple2('__canvas', joakin$elm_canvas$Canvas$cnvs),
				A2(elm$core$List$map, joakin$elm_canvas$Canvas$renderTextureSource, options.aF)));
	});
var joakin$elm_canvas$Canvas$toHtml = F3(
	function (_n0, attrs, entities) {
		...
		return A3(
			...
			{C: h, aF: _List_Nil, H: w},
			attrs,
			entities);
	});
var author$project$Main$view = function (model) {
	var grid = model.h;
	var canvasWidth = grid.H * grid.v;
	var canvasHeight = grid.C * grid.v;
	var background = A2(
		joakin$elm_canvas$Canvas$shapes,
		_List_fromArray(
			[
				joakin$elm_canvas$Canvas$Settings$fill(author$project$Main$colorPalette.ac)
			]),
		_List_fromArray(
			[
				A3(
	...
	var canvas = A3(
		...
		A2(
			elm$core$List$cons,
			background,
			author$project$Main$gameStateView(model)));
	return {
		aJ: _List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('canvón')
					]),
				_List_fromArray(
					[canvas]))
			]),
...
var author$project$Main$main = elm$browser$Browser$document(
	{aZ: author$project$Main$init, be: author$project$Main$subscriptions, bg: author$project$Main$update, bi: author$project$Main$view});
